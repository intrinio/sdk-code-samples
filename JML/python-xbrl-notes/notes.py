...
filing_notes_api = intrinio_sdk.FilingNotesApi()

class IntrinioApp:

    # Calls the `get_all_xbrl_notes` endpoint and follows up to 10 pages worth of data,
    ...
    def getAllNotes(self):
        try:
            # Get the first 10 pages of all notes by repeatedly calling get_all_xbrl_notes with the 
            # next_page value returned from each call.
            print("Calling 'get_all_xbrl_notes")
            print("---------------------------")

            print("Getting the first page of all notes (no 'next_page' parameter passed)")
            api_response = filing_notes_api.get_all_xbrl_notes()
            ...
            while page_count < 10 and api_response.next_page is not None:
                ...
                api_response = filing_notes_api.get_all_xbrl_notes(next_page = api_response.next_page)
                all_notes += api_response.filing_notes
            ...
            print("Exception when calling FilingNotesApi->get_all_xbrl_notes: %s\n" % e)
    ...
    # ValueError: Invalid value for parameter `page_size` when calling `search_xbrl_notes`, must be a value less than or equal to `1000`
    def searchNotes(self, query, page_size):
        try:
            print("Calling 'search_xbrl_notes")
            print("---------------------")

            print("Getting notes with term '{0}'".format(query))
            api_response = filing_notes_api.search_xbrl_notes(query, page_size = page_size)
    ...
    # This calls the `filter_xbrl_notes` endpoint with with company and filing dates (there are others filters that are not
    ...
    def filterNotes(self, company, filing_start_date, filing_end_date):
        try:
            ...
            print("Calling 'filter_xbrl_notes' to get up to 5 pages of notes")
            print("---------------------")

            print("Getting the first page of filtered notes (no 'next_page' parameter passed)")
            api_response = filing_notes_api.filter_xbrl_notes(company=company, filing_start_date = filing_start_date, filing_end_date = filing_end_date)
            ...
            while (page_count < 10 and api_response.next_page is not None):
                ...
                api_response = filing_notes_api.filter_xbrl_notes(company = company, filing_start_date = filing_start_date, filing_end_date = filing_end_date, next_page = api_response.next_page)
                note_count += len(api_response.filing_notes)
                print("There are {0} notes in page {1} with next_page value of {2}".format(len(api_response.filing_notes), page_count, api_response.next_page))
